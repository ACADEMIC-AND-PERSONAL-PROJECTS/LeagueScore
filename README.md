...
- free tier limited to 100 requests/day — sufficient for development but not for sustained live demos
- normalized internal match statuses and events
- local persistence and seed store so the frontend is not coupled to the provider
...
API-Football free plans impose a strict daily request quota (100 requests/day). During development this limit was hit quickly, making real-time synchronization unreliable for demo purposes. The application therefore:
...
- falls back to an internal seed store that supplies realistic Premier League, La Liga, and Bundesliga fixtures, teams, and events for development and screenshots.

## 📸 Application captures

> [Course requirement: Recording a Demo Video](https://github.com/DataTalksClub/ai-dev-tools-zoomcamp/blob/main/cohorts/2026/02-development/homework.md#recording-a-demo-video)

The captures below show the main user journey using **mock data generated by the internal seed store**. The free-tier API-Football plan (100 requests/day) was quickly exhausted during development and could not reliably supply live fixtures for the demo, so the screenshots use the built-in realistic sample dataset instead.

### 1. Live-score home

The home screen presents the live match, score, status, league filter, and available competitions.

![LeagueScore live-score home](./docs/screenshots/01-home-live-center.png)

### 2. Match center

Selecting a live match opens its detailed scoreboard and event timeline.

![LeagueScore match center](./docs/screenshots/02-match-center.png)

### 3. League standings

The standings view provides season selection and league-specific rankings.

![LeagueScore standings](./docs/screenshots/03-standings.png)

### 4. Administrator access

The administrator area is protected by a dedicated login screen for match and event management.

![LeagueScore administrator login](./docs/screenshots/04-admin.png)
